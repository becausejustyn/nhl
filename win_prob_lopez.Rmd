...
Assume home team $i$ (listed at -130 on the money line) is playing away team $j$ (+110). Punters backing $i$
believe that squad has better than a $\frac{130}{230} = 56.5%$ chance of winning, while betters on team 
$j$ assume it has at least a $\frac{100}{210} = 47.6%$ probability of winning. Accounting for the vig – which markets use to ensure a long-term profit – team $i$ can be assumed to have a $54.3%$ chance of beating $j$.
...
tab.out <- head(nhl1718, 4) %>% 
  select(date, home, away, p_home)
...
hfas <- data.frame(round(z$alpha[,,], 3))  %>% 
  mutate(draw = 1:n())
...
teamcolors1 <- teamcolors %>% 
  ...
  rename(team_id = name) %>% 
  mutate(
    rand.color = ifelse(primary == "#010101", secondary, primary))

df.beta1 <- df.beta %>% 
  left_join(teamcolors1, by = c("team_id" = "team_id"))
```
